...
def run_streamsat_pipeline(
    ...
    if rc != 0:
        ...
        raise RuntimeError(f"STREAM-Sat pipeline failed for {domain} (exit {rc})")
...
def parse_nc_timestamp(nc_path: str) -> str | None:
...
def convert_streamsat_nc_to_geotiffs(
    ...
    if dry_run:
        log.info(
            "[DRY RUN] Would create %d GeoTIFFs in %s/ensP1..ensP%d",
            total_tifs,
            tif_dest_root,
            n_ensemble,
        )
...
def get_streamsat_time_range(nc_output_dir: str) -> tuple[datetime | None, datetime | None]:
