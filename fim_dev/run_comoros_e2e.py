...
def main():
    ...
    for region, pcode, island in SITES:
        ...
        ck(
            f"{region}: pluvial only",
            s["hazards_enabled"] == {"pluvial": True, "fluvial": False},
        ...
        if st:
            mag = mm[sid.index(st)]
            ck(
                f"{region}: match inside the 0.9 to 1.2 band",
                0.9 * tot <= mag <= 1.2 * tot,
                f"{st} at {mag:.1f} mm for {tot:.1f} mm",
            )
        d = f"outputs/_test/{region}/pluvial"
        prods = sorted(os.listdir(d)) if os.path.isdir(d) else []
        ck(
            f"{region}: four probability products",
            all(
        ...
        ck(
            ...
            os.path.isdir(f"outputs/_test/{region}/pluvial_overbank"),
        )

        with rasterio.open(os.path.join(d, f"prob_depth_ge_10cm.{CYCLE}.tif")) as ds:
            P = ds.read(1)
            ck(
                f"{region}: products carry the island model grid and crs",
                str(ds.crs) == z.attrs["crs"] and list(ds.shape) == list(z.attrs["grid_shape"]),
                f"{ds.crs} {ds.shape}",
            )
        ck(
            f"{region}: the flood map is not empty",
            float(P.max()) > 0,
            f"max prob {float(P.max()):.2f}, wet px {(P > 0).sum():,}",
        )
        ob = os.path.join(
            f"outputs/_test/{region}/pluvial_overbank", f"prob_depth_ge_10cm_overbank.{CYCLE}.tif"
        )
        with rasterio.open(ob) as ds:
            obn = ds.read(1)
        ck(
            f"{region}: overbank mask removes at most the baseline water",
            int((P > 0).sum()) - int((obn > 0).sum()) >= 0
            and (int((P > 0).sum()) - int((obn > 0).sum())) <= 0.10 * max(1, int((P > 0).sum())),
            f"{(P > 0).sum() - (obn > 0).sum():,} px removed of {(P > 0).sum():,}",
        )
