...
def test_operational_stream_sat_windows():
    ...
    assert names == ["stream_sat", "scampr_qpf"]
    ...
    assert gap.end == plan.lr_end
    assert gap.qpe_source == "SCAMPR"
    assert gap.qpf_sources == ("GFS", "AROME")
    assert gap.state_load_time == plan.qpe_end
...
def test_operational_stream_sat_data_driven_ss_end():
    ...
    assert gap.end == T + timedelta(hours=24)


# ── Hindcast STREAM_SAT ────────────────────────────────────────────────────

def test_hindcast_stream_sat_no_latency_gfs_only():
    plan = build_cycle_plan(
        T,
        mode="hindcast",
        qpe_source="STREAM_SAT",
    ...
    assert plan.phases[1].qpe_source == "STREAM_SAT"  # no SCaMPR
...
def test_operational_state_chaining_hour_to_hour():
    """Operational: hour N saves at T−4h; hour N+1 finds ~(T+1)−4h = T−3h? 

    Careful: each cycle re-runs STREAM-Sat over a sliding 48h window ending
    near its own T−4h.  The state saved at cycle T is at T−4h.  Cycle T+1h
    looks for states near (T+1h)−4h = T−3h, but will accept any state within
    48h lookback — so T−4h from previous cycle is still valid and is the
    warm-start seed until the new STREAM-Sat phase overwrites it.
    """
    t0 = datetime(2026, 7, 22, 12, 0)
    t1 = t0 + timedelta(hours=1)

    plan0 = build_cycle_plan(t0, mode="operational", qpe_source="STREAM_SAT")
    saved = plan0.primary_state_time
    assert saved == t0 - IMERG_LATENCY

    expected = expected_state_for_next_cycle(plan0, t1)
    assert expected == saved

    plan1 = build_cycle_plan(t1, mode="operational", qpe_source="STREAM_SAT")
    assert plan1.state_lookback_start <= expected
    # New cycle's own ss_end advances by 1h
    assert plan1.qpe_end == t1 - IMERG_LATENCY
    assert plan1.qpe_end == saved + timedelta(hours=1)
...
def test_guatemala_gfs_only_qpf():
    plan = build_cycle_plan(
        T,
        mode="operational",
        qpe_source="STREAM_SAT",
        qpf_sources=["GFS"],  # Guatemala has no AROME
    )
    assert plan.phases[1].qpf_sources == ("GFS",)
...
def test_no_lr_only_stream_sat_phase():
...
def test_phases_are_contiguous_for_stream_sat_operational():
    plan = build_cycle_plan(T, mode="operational", qpe_source="STREAM_SAT")
    ss, gap = plan.phases
...
def test_primary_state_is_last_save():
    plan = build_cycle_plan(T, mode="operational", qpe_source="STREAM_SAT")
    assert plan.primary_state_time == plan.phases[0].state_save_time
