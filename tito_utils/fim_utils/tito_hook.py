...
Output layout (cycle-first, chain-tagged)::
  ...
  outputs/<cycle>/<rkey>/fim/<chain>/<Site>/   per-site FIM products
  outputs/<cycle>/<rkey>/fim/<chain>/<mode>/   mosaic over all sites (max)
    e.g. outputs/20230621.070000/guatemala_90m/fim/stream_sat_stormlab/SantaInesPetapa/
         outputs/20230621.070000/guatemala_90m/fim/stream_sat_stormlab/pluvial_overbank/
...
def run_fim_for_cycle(
    *,
    ...
    for cr in sorted(chain_roots):
        ...
        if n:
            log(f"  FIM: mosaicked {n} product(s) → {cr}")
            if master_log:
