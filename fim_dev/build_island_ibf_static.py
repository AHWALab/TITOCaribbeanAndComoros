...
def build_country(job, out_root):
    ...
    keep = ["ADM1_PCODE", name_col, "population", "pop_year", "pop_source", "geometry"]
    adm_out = adm[[c for c in keep if c in adm.columns]].rename(columns={name_col: "ADM1_EN"})
    ...
    log(
        f"  {country}: admin {len(adm_out)} units, "
    ...
    bld = gpd.read_parquet(bldg_pq, columns=["id", "subtype", "class", "height", "geometry"])
    bld = bld[bld.geometry.notna() & ~bld.geometry.is_empty].copy()
    seg = gpd.read_parquet(seg_pq, columns=["id", "subtype", "class", "geometry"])
    roads = seg[(seg["subtype"] == "road") & seg.geometry.notna() & ~seg.geometry.is_empty][
        ["id", "class", "geometry"]
    ].copy()
    ...
    manifest["counts"].update(
        {
            ...
            "road_class_top": {k: int(v) for k, v in roads["class"].value_counts().head(8).items()},
        }
    )
    log(
        f"  {country}: {len(bld):,} buildings ({n_sub:,} with subtype), "
    ...
    if ghs_zips:
        ...
        for i, ghs_zip in enumerate(ghs_zips):
            with zipfile.ZipFile(ghs_zip) as z:
                member = [n for n in z.namelist() if n.lower().endswith(".tif")][0]
        ...
        try:
            ...
            shapes = [g.__geo_interface__ for g in geoms.values if not g.is_empty]
            minx, miny, maxx, maxy = gpd.GeoSeries(geoms, crs=srcs[0].crs).total_bounds
            if len(srcs) > 1:
                data, transform = rio_merge(
                    srcs, bounds=(minx, miny, maxx, maxy), nodata=srcs[0].nodata
                )
            ...
            data, transform = rio_mask(
                mask_src, shapes, crop=True, all_touched=True, nodata=mask_src.nodata
            )
        ...
        manifest["counts"]["ghs_value_histogram"] = {
            str(int(v)): int(c) for v, c in zip(vals, counts, strict=False)
        }
        log(
            f"  {country}: GHS BUILT-C crop {data.shape[2]}x{data.shape[1]} "
