...
def run_fim_cycle(config, cycle: str = None, verbose: bool = True) -> dict:
    ...
    aocs = load_aocs(
        config.resolve(config.aoc.source),
    ...
    ef5 = config.ef5
    runs = discover_runs(
        config.resolve(ef5.outputs_root),
        config.region,
        cycle=cycle,
        run_dir_glob=ef5.run_dir_glob,
        cycle_format=ef5.cycle_format,
        file_templates={
            "uq": ef5.files.uq,
            "qpe_accum": ef5.files.qpe_accum,
            "qpf_accum": ef5.files.qpf_accum,
            "max_q": ef5.files.max_q,
        },
        qpe_sources=ef5.qpe_sources,
        qpf_sources=ef5.qpf_sources,
        require_qpf=ef5.require_qpf,
    )

    if not runs:
        log(f"  FIM {config.region}: no EF5 runs found (cycle={cycle or 'latest'}); nothing to do")
        return {"region": config.region, "cycle": cycle, "status": "no_runs"}

    cycle = runs[0].cycle
    log(
        f"  FIM {config.region} cycle {cycle}: {len(runs)} member(s): "
        + ", ".join(r.member_id for r in runs)
    )

    # 1. Trigger on EF5 max unit streamflow
    trig = evaluate_trigger(
        runs, aocs, threshold=config.trigger.uq_threshold, scope=config.trigger.scope
    )
    if not trig.triggered:
        log(
            f"  FIM {config.region}: quiet (max UQ below "
    ...
    summary = export_products(
        ...
        config_echo={
            ...
            "band": list(rules.band),
            "band_wide": list(rules.band_wide),
    ...
    for aoc_id, named in selections.items():
        if aoc_id in trig.triggered_aocs:
            parts = [
                f"{sel}={d.storm_id} ({d.storm_magnitude_mm:.0f} mm, {d.rule_applied})"
...
def main(argv=None):
    ...
    parser.add_argument("--cycle", default=None, help="Cycle timestamp (default: latest on disk)")
    parser.add_argument("--root", default=None, help="Repo root for relative paths (default: cwd)")
