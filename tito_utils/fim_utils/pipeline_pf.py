...
from .pluvial import member_rain_total, match_pluvial
...
def load_pf_config(path: str, root: str = None) -> dict:
    ...
    hz["pluvial"].setdefault("grid", "qpe_accum")
...
def run_pf_cycle(cfg, cycle: str = None, hazard_filter: str = None,
    ...
    out_dir = os.path.join(_resolve(cfg, cfg["products_root"]), cycle)
    ...
    for m in members:
        ...
        if hz_p["enabled"]:
            total, pfl = member_rain_total(
                run_dir, cycle, bounds, files, hz_p["grid"],
                sampling["expand_steps_km"], sampling["min_valid_cells"])
