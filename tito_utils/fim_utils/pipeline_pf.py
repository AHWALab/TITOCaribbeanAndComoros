...
from .pluvial import match_pluvial, member_rain_total, member_rain_total_components
...
def run_pf_cycle(cfg, cycle: str = None, hazard_filter: str = None, verbose: bool = True) -> dict:
    ...
    log(
        f"  PF FIM {cfg['region']} {cycle}: {len(members)} members "
    ...
    for m in members:
        for src in cfg["trigger"]["sources"]:
            ...
            if not up:
                continue
            zs = zone_stat(
                up, bounds, "max", sampling["expand_steps_km"], sampling["min_valid_cells"]
            )
            rows_trig.append(
                {
                    "source": os.path.relpath(rd, outputs_root).replace(os.sep, "/"),
    ...
    log(
        f"    trigger: max UQ {max_uq:.2f} vs {threshold} -> "
    ...
    rows, pairs = [], []
    for m in members:
        run_dir = resolve_component_dir(outputs_root, cfg["member"]["template"], m.keys, cycle)
        ...
        if hz_p["enabled"]:
            ...
            if rain_comps:
                ...
                for pname, pval in parts.items():
                    row[f"rain_{pname}"] = None if pval is None else round(float(pval), 2)
            else:
            ...
            row.update(
                {
                    "total_mm": None if total != total else round(total, 2),
                    "p_storm": dec["storm_id"],
                    "p_rule": dec["rule_applied"],
                    "p_flags": ";".join(sorted(set(pfl + dec["flags"]))),
                }
            )
        if hz_f["enabled"]:
            qvec, qfl = member_boundary_q(run_dir, cycle, hz_f["boundary_series"], hz_f["stat"])
            fdec = fmatch.match(qvec, hz_f["method"])
            f_idx = fdec["storm_index"]
            for name, val in zip(
                fmatch.names or [f"Q{i + 1}" for i in range(len(qvec))], qvec, strict=False
            ):
                row[name] = None if val != val else round(float(val), 2)
            row.update(
                {
                    "f_storm": fdec["storm_id"],
                    "f_distance": fdec["distance"],
    ...
    for mode in modes:
        ...
        for t in thresholds:
            ...
            stats[_tag(t)] = {
                "max_probability": round(float(p.max()), 3),
        ...
        log(
            f"    {mode:2s}: n={n_used} "
            + " ".join(
                f"{k}:maxP={v['max_probability']} px={v['pixels_gt0']}" for k, v in stats.items()
            )
        )
