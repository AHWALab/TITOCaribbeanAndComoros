...
def WRF_searcher(
    path_wrf, qpf_store_path, start_time, end_time, LR_timestep, var_name, filename_template
):
    """
    ...
    if not missing_files:
        ...
        conversion_ok = True
        for f, t in zip(expected_files, expected_times, strict=False):
            try:
                netcdf_to_geotiff(f, download_folder, var_name)
                # Rename to EF5-compatible format: wrf.YYYYMMDDHH00.tif
                orig_tif = os.path.join(
                    download_folder, os.path.basename(f)[: -len(".nc")] + ".tif"
                )
                std_tif = os.path.join(download_folder, f"wrf.{t:%Y%m%d%H%M}.tif")
                if os.path.exists(orig_tif) and os.path.abspath(orig_tif) != os.path.abspath(
                    std_tif
                ):
        ...
        print("⚠️ WARNING ⚠️ The system will run without WRF inputs due to missing data.")
        print("⚠️ Check tito_utils/qpf_utils/wrf_manager.py for details.")
        return False
