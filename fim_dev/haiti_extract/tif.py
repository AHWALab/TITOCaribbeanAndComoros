...
def tags(buf):
    ...
    if big:
        ...
        (n,) = struct.unpack(bo + "Q", buf[off : off + 8])
        p = off + 8
        esz = 20
    else:
    ...
    for i in range(n):
        ...
        if big:
            ...
            (cnt,) = struct.unpack(bo + "Q", e[4:12])
            val = e[12:20]
        else:
            tag, typ = struct.unpack(bo + "HH", e[:4])
            (cnt,) = struct.unpack(bo + "I", e[4:8])
            val = e[8:12]
        sz = TS.get(typ, 1) * cnt
        if sz > len(val):
...
def describe(buf):
    ...
    d = {
        "width": t[256][0],
        "height": t[257][0],
        "bits": t.get(258, [None])[0],
        "sample_format": t.get(339, [1])[0],
        "compression": t.get(259, [1])[0],
        "photometric": t.get(262, [None])[0],
        "planar": t.get(284, [1])[0],
        "samples": t.get(277, [1])[0],
        "bigtiff": big,
        "byteorder": bo,
    }
    if 322 in t:
        d["tile"] = (t[322][0], t[323][0])
        d["ntiles"] = len(t[324])
    if 278 in t:
        d["rows_per_strip"] = t[278][0]
        d["nstrips"] = len(t[273])
    if 33550 in t:
        d["pixel_scale"] = t[33550]
    if 33922 in t:
        d["tiepoint"] = t[33922]
    if 42113 in t:
        d["nodata"] = t[42113]
    if 34737 in t:
        d["geo_ascii"] = t[34737][:120]
    if 34735 in t:
        d["geo_keys"] = t[34735][:40]
    d["_t"] = t
    return d
...
def read_float(buf):
    ...
    mv = memoryview(buf)
    for i, (o, _c) in enumerate(zip(off, cnt, strict=False)):
        r0 = i * rps
        rows = min(rps, H - r0)
