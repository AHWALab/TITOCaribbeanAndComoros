...
def main():
    ...
    for r in rows:
        ...
        ck(
            f"{tag}: 200 scenarios",
            depth.shape[0] == 200 == len(sid) == len(mm),
        ...
        ck(
            f"{tag}: dtypes",
            str(depth.dtype) == "float32" and str(extent.dtype) == "uint8",
        ...
        ck(
            f"{tag}: admin_unit attr",
            at["admin_unit"]["pcode"] == pcode and at["admin_unit"]["name"] == r["name"],
        )
        ck(
            f"{tag}: fluvial index absent (pluvial only country)",
            "fluvial_q" not in names and not any(n.startswith("fluvial_q") for n in names),
        )

        # index.csv and meta.json are the human readable companions
        ck(
            f"{tag}: index.csv rows match the arrays",
            [x["storm_id"] for x in icsv] == sid
            and np.allclose([float(x["magnitude_mm"]) for x in icsv], mm, atol=0.005),
        )
        ck(
            f"{tag}: meta.json matches attrs",
            meta["grid_shape"] == list(at["grid_shape"]) and meta["n_storms"] == at["n_storms"],
        )

        # magnitudes must be the ones computed from the rain geotiffs
        src_m = mags_all[pcode]
        ck(
            f"{tag}: magnitudes come from the rain table",
            np.allclose(mm, [src_m[s] for s in sid], rtol=0, atol=1e-9),
        )
        ck(
            f"{tag}: order is the magnitude sort of that table",
            sid == sorted(src_m, key=lambda s: src_m[s]),
        )
        ck(
            f"{tag}: manifest magnitude range",
            abs(float(r["magnitude_min_mm"]) - mm.min()) < 0.05
        ...
        ck(
            f"{tag}: stored transform equals the recomputed window transform",
            np.allclose(at["transform"], list(np.asarray(wt)[:6]), atol=1e-9),
        )
        ck(
            f"{tag}: window size equals the array size",
        ...
        ck(
            f"{tag}: municipality covered by the island model grid",
            cov >= 99.99 or pcode in KNOWN_PARTIAL,
            f"{cov:.2f} percent",
        )
        ...
        for k in picks:
            ...
            if not np.array_equal(got, want):
                ck(
                    f"{tag}: depth[{k}] equals the source window",
                    False,
        ...
        ck(f"{tag}: yaml outputs_root is the orchestrator root", "outputs_root: outputs\n" in y)
        ck(f"{tag}: yaml store points at this store", f"store: fim_store/Comoros/{r['store']}" in y)
        ck(
            f"{tag}: yaml thresholds are the standard four",
        ...
        ov = [
            ...
            if ln.strip().startswith("reference_scenario")
        ][0]
        ...
        ck(
            f"{tag}: overbank mask covers only baseline water",
            wet0 < 5.0,
            f"{wet0:.2f} percent of the window at 0.10 m",
        )
        ck(
            f"{tag}: aoc file referenced exists",
        ...
        f0 = gj["features"][0]
        ck(
            f"{tag}: aoc properties",
            f0["properties"]["pcode"] == pcode and f0["properties"]["country"] == "Comoros",
        )
        ...
        ck(
            f"{tag}: aoc geometry equals the shapefile polygon",
            bool(agj.geometry.iloc[0].equals(gs.iloc[0])),
        )
        ck(
            f"{tag}: aoc has no degenerate part",
    ...
    print(
        "overbank mask, largest baseline wet fraction at 0.10 m: "
