...
def get_gpm_files(precipFolder, initial_timestamp, final_timestamp, ppt_server_path, email_gpm, xmin, ymin, xmax, ymax, HindCastMode=False):
    """Batch download IMERG files for a given time range.
    ...
    # Pre-fetch available files from server for the date range
    print(f"    Checking server for available files...")
    available_files = _get_available_files_for_range(server, email_gpm, initial_timestamp, final_date, HindCastMode)
    ...
    while (current_date < final_date):
        ...
        # Check if file exists on server before trying to download
        if filename not in available_files:
            print(f'    SKIP (not on server): {final_time_gridout.strftime("%Y-%m-%d %H:%M")}')
            skipped_count += 1
            current_date = current_date + delta_time
            continue

        print(f'    Downloading: {final_time_gridout.strftime("%Y-%m-%d %H:%M")}')
        # Download raw file to a per-region subfolder to prevent parallel
        # region threads from reading/deleting each other's raw downloads.
        ...
        try:
            # Download from NASA server directly to the per-region raw folder.
            get_file(filename, server, email_gpm, local_path=local_file_path)
            # Process file for domain and to fit EF5.
            # Filename has final datestamp as it represents the accumulation up to that point.
            ...
            # Write processed file and clean up raw download.
    ...
    print(f"    Download complete: {downloaded_count} downloaded, {skipped_count} skipped/missing")
