...
class ModelPicker:
    ...
    def load_model(self, get_ensemble=True):
        ...
        if self.model_type == 'naive':
            ...
            if self.img_height != self.input_precip.shape[1]:
                h_start = (self.input_precip.shape[1] - self.img_height) // 2
                self.input_precip = self.input_precip[:, h_start:h_start+self.img_height, :]
        
            if self.img_width != self.input_precip.shape[2]:
                w_start = (self.input_precip.shape[2] - self.img_width) // 2
                self.input_precip = self.input_precip[:, :, w_start:w_start+self.img_width]
            ...
            Y = torch.zeros(self.input_precip.shape, dtype=torch.float32, device=device)
    ...
    def predict(self, samples = None, samples_ir = None):
        ...
        if self.model_type in ['naive', 'linda', 'steps', 'lagrangian']:
            ...
            # convert to tensor
            with torch.no_grad():
                pred_Y = self.prediction_function(self.input_precip[-self.config['in_seq_length']:])
            # convert to numpy
