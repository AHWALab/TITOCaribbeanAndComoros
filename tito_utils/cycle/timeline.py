...
----------------------------------------
    ...
    Phase B gap+LR:      ss_end           →  T + 24 h
                         SCaMPR QPE fills ss_end → T
                         QPF (GFS/AROME) fills T → T+24h
                         states NOT saved
...
--------------------------------------
    ...
    Phase H QPF-only:    T → T+24h with GFS only
                         states NOT saved
    Next hour (T+1):     loads states saved at ~T
...
def _stream_sat_phases(
    *,
    ...
    phases = [
        PhaseWindow(
            ...
            notes=(
                ...
                if mode == "hindcast"
                else "Operational: ss_end driven by last STREAM-Sat GeoTIFF (~IMERG latency)."
            ),
        )
    ]
    ...
    if mode == "hindcast":
        # AROME has no archive → GFS only
        hindcast_qpf = tuple(s for s in qpf if s == "GFS") or ("GFS",)
        phases.append(
            PhaseWindow(
                name="hindcast_qpf",
                start=ss_end,
                ...
                qpe_source="STREAM_SAT",
                qpf_sources=hindcast_qpf,
                save_states=False,
                state_load_time=ss_end,
                notes="QPF-only from STREAM-Sat states; no SCaMPR gap fill.",
            )
        )
    else:
        phases.append(
            PhaseWindow(
                ...
                save_states=False,
                state_load_time=ss_end,
                notes="SCaMPR fills ss_end→T; QPF fills T→T+LR.",
            )
        )
