...
class Catalog:
    ...
    def storm_files(self, storm_id: str) -> dict:
        ...
        return {
            ...
            "extent": os.path.join(self.folder, row["extent_path"])
            if row.get("extent_path")
            else "",
        }
    ...
    def load(cls, folder: str) -> "Catalog":
        ...
        if "direction_deg" not in index.columns:
            ...
            index["direction_deg"] = (
                index[source_col].map(direction_to_deg) if source_col else float("nan")
            )
...
def build_catalog(
    ...
    for _, meta_row in meta_df.iterrows():
        ...
        depth_matches = sorted(
            glob.glob(os.path.join(maps_dir, depth_pattern.format(storm_id=storm)))
        )
        if not depth_matches:
            problems.append(
                f"{storm}: no depth raster matching {depth_pattern.format(storm_id=storm)}"
            )
            continue
        ...
        extent_rel = ""
        extent_matches = sorted(
            glob.glob(os.path.join(maps_dir, extent_pattern.format(storm_id=storm)))
        )
        ...
        if rain_dir:
            rain_matches = sorted(
                glob.glob(os.path.join(rain_dir, rain_pattern.format(storm_id=storm)))
            )
            if rain_matches:
                ...
                problems.append(
                    f"{storm}: no rainfall raster matching {rain_pattern.format(storm_id=storm)}"
                )
        if magnitude != magnitude:
    ...
    if by_aoc_rows:
        pd.DataFrame(by_aoc_rows).to_csv(
            os.path.join(out_dir, "magnitudes_by_aoc.csv"), index=False
        )
    ...
    if problems:
        ...
        for p in problems:
            print(f"    - {p}")
    print(
        f"  Catalog built: {len(index)} storms, "
...
def main(argv=None):
    ...
    parser.add_argument(
        "--storm-meta", required=True, help="CSV with storm_id[, magnitude_mm, direction]"
    )
    parser.add_argument("--maps-dir", required=True, help="Folder with max-depth rasters")
    parser.add_argument(
        "--out", required=True, help="Output catalog folder (e.g. fim_catalog/Barbados/v1)"
    )
    parser.add_argument(
        "--rain-dir", default=None, help="Optional folder with per-storm rainfall rasters"
    )
