...
def read_depth(outer_path, info):
    with open(outer_path, "rb") as f:
        ...
        doff, csize = nested_member_span(f, nbase, info.file_size, "MaximumDepth.tif")
        f.seek(doff)
        comp = f.read(csize)
...
def main():
    ...
    ap.add_argument(
        "--split-mb", type=int, default=0, help="split the zip into parts of this many MB"
    )
    a = ap.parse_args()
        ...
        with MemoryFile(raw) as mf, mf.open() as src:
            ...
            if ref is None:
                ...
                crs, transform, nodata = (
                    str(src.crs),
                    tuple(np.asarray(src.transform)[:6]),
                    src.nodata,
                )
                ny, nx = src.height, src.width
        if sig != ref:
            print(f"  SKIP {sample}: grid {sig[0]}x{sig[1]} differs from {ref[0]}x{ref[1]}")
            continue
        arr[arr < 0] = 0.0
    ...
    if real_mags:
        ...
        if missing:
            raise SystemExit(f"magnitudes CSV misses {len(missing)} samples, e.g. {missing[:3]}")
        ...
        source = (
            "PLACEHOLDER wet-volume RANKS (1..N), NOT millimeters; "
