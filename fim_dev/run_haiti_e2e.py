...
RKEY = "haiti_90m"
SITES = [
    ("Haiti_Gris", [f"ts.cuenca_griss.crest.{CYCLE}.csv"]),
    (
        "Haiti_LaQuinte",
        [
            f"ts.cuenca_laquinta_1.crest.{CYCLE}.csv",
            f"ts.cuenca_laquinta_2.crest.{CYCLE}.csv",
        ],
    ),
]
...
def grid(path, b, value, res=0.002, pad=0.05):
    ...
    w = max(20, int((maxx - minx) / res))
    h = max(20, int((maxy - miny) / res))
...
def main():
    ...
    for region, gauges in SITES:
        ...
        for g, q in zip(gauges, qprobe, strict=False):
        ...
        ck(
            f"{region}: both hazards enabled",
            s["hazards_enabled"] == {"pluvial": True, "fluvial": True},
            str(s.get("hazards_enabled")),
        )
        ck(
            f"{region}: P, F and PF routines all ran",
            set(s.get("routines", {})) >= {"P", "F", "PF"},
            str(sorted(s.get("routines", {}))),
        )

        r = list(csv.DictReader(open(f"outputs/_test/{region}/member_decisions.csv")))[0]
        tot = float(r["total_mm"])
        ck(
            f"{region}: areal rain read back as the probe total",
            abs(tot - target) / target < 0.02,
            f"{tot:.1f} vs {target:.1f} mm",
        )
        ck(
            f"{region}: pluvial matched",
            r.get("p_storm", "") != "",
            f"{r.get('p_storm')} rule {r.get('p_rule')}",
        )
        if ph:
            ck(
                f"{region}: the matched scenario IS a placeholder (stand in map works)",
                r.get("p_storm") in ph,
                str(r.get("p_storm")),
            )
        if r.get("p_storm"):
            ...
            ck(
                f"{region}: pluvial match inside the band",
                0.9 * tot <= mag <= 1.2 * tot,
                f"{mag:.1f} mm for {tot:.1f} mm",
            )
        ck(
            f"{region}: fluvial matched on the warm up series (nan safe)",
        ...
        for mode in ("pluvial", "fluvial", "combined"):
            ...
            ck(
                f"{region}: {mode} has four probability products",
                all(
                    any(f"prob_depth_ge_{t}." in p for p in prods)
                    for t in ("10cm", "30cm", "70cm", "100cm")
                ),
                f"{len(prods)} files",
            )
        ...
        with rasterio.open(pp) as a, rasterio.open(fp) as bb, rasterio.open(cp) as c:
            ...
            ck(
                f"{region}: products carry the model grid and crs",
                str(c.crs) == z.attrs["crs"] and list(c.shape) == list(z.attrs["grid_shape"]),
                f"{c.crs} {c.shape}",
            )
        ck(
            f"{region}: combined is the per pixel maximum of P and F",
            np.allclose(C, np.maximum(P, F), atol=1e-6),
            f"maxdiff {float(np.abs(C - np.maximum(P, F)).max()):.4f}",
        )
        ck(
            f"{region}: pluvial product is not empty",
            float(P.max()) > 0,
            f"max {float(P.max()):.2f}",
        )
        ck(
            f"{region}: fluvial product is not empty",
            float(F.max()) > 0,
            f"max {float(F.max()):.2f}",
        )
        ck(
            f"{region}: combined wet area at least the pluvial wet area",
            int((C > 0).sum()) >= int((P > 0).sum()),
            f"P {(P > 0).sum():,} F {(F > 0).sum():,} C {(C > 0).sum():,} px",
        )
        ck(
            f"{region}: overbank products written",
