...
SITE = sys.argv[1]
st = json.load(open(os.path.join(B, f"state_{SITE.lower()}.json")))["done"]
ck_path = os.path.join(B, f"verify_archive_{SITE.lower()}.json")
...
for smp in sorted(st):
    ...
    if time.time() - t0 > BUDGET:
        print(f"budget reached ({len(done)} verified so far), rerun", flush=True)
        json.dump(done, open(ck_path, "w"))
        sys.exit(0)
    p = os.path.join(OUT, SITE, f"{smp}_MaximumDepth.tif")
    buf = open(p, "rb").read()
    bo, big, t = tif.tags(buf)
    H, W = t[257][0], t[256][0]
    rps = t[278][0]
    rows = []
    for i, (o, c) in enumerate(zip(t[273], t[279], strict=False)):
...
print(
    f"DONE {SITE}: {n_ok} of {len(st)} verified pixel exact, {len(st) - n_ok} BAD "
    f"{[k for k, v in done.items() if not v] or ''}",
    flush=True,
)
