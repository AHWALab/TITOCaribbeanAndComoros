...
SITES = {
    "Gris": {
        ...
        "zip": "HAITI_outputs_Q_GRISS",
        "gauges": ["ts.cuenca_griss.crest.csv"],
    },
    "LaQuinte": {
        "key": "laquinte",
        "grid": (3557, 3417),
...
def main():
    ...
    for site, cfg in SITES.items():
        ...
        ck(
            f"{site}: ALL 200 scenarios are in the store",
            set(sid) == set(mags),
        ...
        ck(
            f"{site}: placeholder list is exactly the undelivered scenarios",
        ...
        ck(
            f"{site}: shapes",
            depth.shape == (n, H, W) and extent.shape == (n, H, W),
            str(depth.shape),
        )
        ck(
            f"{site}: depth is uint16 centimetres with scale 0.01",
            str(depth.dtype) == "uint16"
            and at.get("depth_scale") == 0.01
            and str(extent.dtype) == "uint8",
        )
        ck(
            f"{site}: crs and transform",
            at["crs"] == "EPSG:32618"
            and np.allclose(
                at["transform"], [2.0, 0.0, cfg["origin"][0], 0.0, -2.0, cfg["origin"][1]]
            ),
        ...
        ck(
            f"{site}: fluvial index matches the delivered discharge, in store order",
            fq.shape == want.shape and np.allclose(fq, want, equal_nan=True),
            f"{fq.shape} max {np.nanmax(fq):.0f} m3/s",
        )
        ck(
            f"{site}: fluvial mu and sigma",
            np.allclose(at["fluvial_mu"], fq.mean(axis=0))
            and np.allclose(at["fluvial_sigma"], fq.std(axis=0, ddof=1)),
        )
        ck(
            f"{site}: no gauge is all nan",
            bool(np.isfinite(fq).all()),
        ...
        for k in picks:
            ...
            if not np.array_equal(
                np.asarray(extent[k]), (want_d >= EXTENT_M * 100).astype("uint8")
            ):
                bad = (k, "extent")
                break
        ck(
            f"{site}: real depth and extent equal the delivered maps ({len(picks)} scenarios)",
            bad is None,
        ...
        if ph_idx:
            ...
            for k in random.sample(ph_idx, min(8, len(ph_idx))):
                ...
                if not np.array_equal(np.asarray(depth[k]), ph_map):
                    ...
                    break
            ck(
                f"{site}: placeholder scenarios carry exactly {ph}'s map "
                f"(8 of {len(ph_idx)} checked)",
                bad is None,
                str(bad) if bad else "",
            )
            stat = {r["storm_id"]: r["map_status"] for r in icsv}
            ck(
                f"{site}: index.csv map_status marks every placeholder",
                all(stat[s0] == f"placeholder_copy_of_{ph}" for s0 in want_ph)
                and all(stat[s0] == "real" for s0 in sid if s0 in st),
            )

        ck(
            f"{site}: index.csv agrees with the arrays",
            [r["storm_id"] for r in icsv] == sid
            and np.allclose([float(r["magnitude_mm"]) for r in icsv], mm, atol=0.005),
        )
        ck(f"{site}: meta.json agrees", meta["n_storms"] == n and meta["grid_shape"] == [H, W])

        # the overbank reference must be a dry scenario
        d0 = np.asarray(depth[0]).astype("float32") * 0.01
        wet0 = float((d0 >= 0.10).mean()) * 100
        ck(
            f"{site}: driest scenario is dry enough to be the overbank reference",
            mm[0] < 1.0 and wet0 < 5.0,
            f"{mm[0]:.2f} mm, {wet0:.3f} percent wet at 0.10 m",
        )

        print(
            f"   store {os.path.getsize(zp) / 1e6:.1f} MB, {n} scenarios, "
