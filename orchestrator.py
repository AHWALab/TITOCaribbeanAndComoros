...
from tito_utils.ef5.jobs.helpers import region_path_key, resolve_region_resolutions
...
def main(args):
    ...
    _ap.add_argument(
        "--regions", default=None, help="Comma-separated region names (overrides config)"
    )
    _ap.add_argument(
        "--hindcast-date",
        default=None,
    ...
    _cli_regions = (
        [r.strip() for r in _cli.regions.split(",") if r.strip()] if _cli.regions else None
    )
    ...
    if is_user():
        user_print(
            "TITO ready  (console=user; set console_verbosity='debug' or "
            "--debug-console for full logs)"
        )
    ...
    def _normalize_qpf(val, fallback):
        if isinstance(val, (list, tuple)):
            return [v for v in (str(x).strip().upper() for x in val) if v in _VALID_QPF] or [
                fallback
            ]
        c = str(val).strip().upper()
    ...
    if HindCastMode:
        ...
        if hc_end_str:
            ...
            while t <= hc_end:
                ...
                t += timedelta(hours=1)
            console.info(
                "[bold]HINDCAST LOOP[/] %s → %s (%d cycles)",
                hc_start.strftime("%Y-%m-%d %H:%M"),
                hc_end.strftime("%Y-%m-%d %H:%M"),
                len(hc_dates),
            )
        else:
            hc_dates = [hc_start]
            console.info(
                "[bold]HINDCAST MODE[/] — single cycle: %s", hc_start.strftime("%Y-%m-%d %H:%M")
            )
        ...
        cycle_time = round_cycle_time(datetime.now(UTC).replace(tzinfo=None), systemTimestep)
...
def _run_single_cycle(
    ...
    master_log = setup_run_log(log_root, f"pipeline_{cycle_time.strftime('%Y%m%d_%H%M')}")
    ...
    # Build the (region, resolution) run pairs from region_resolution_map.
    resolution_map = getattr(config, "region_resolution_map", {})
    pairs = []
    for r in regions_to_run:
        for res in resolve_region_resolutions(r, model_resolution, resolution_map):
            pairs.append((r, res))
    if not pairs:
        pairs = [(r, model_resolution) for r in regions_to_run]
    ...
    # STEP 2 — Precipitation (once per domain; resolution-independent)
    ...
    if is_debug():
        console.info("[bold]STEP 2:[/] Precipitation ready. %s", summarize_shared_precip(shared))
    else:
        console.info("[bold]STEP 2:[/] Precipitation ready.")

    # Run warmup + EF5 + FIM once per (region, resolution) pair, sharing the
    # precip prepared above. States/outputs stay split by {region}_{resolution}.
    for r, res in pairs:
        res_map = {r: res}
        console.rule(f"[bold]{r} @ {res}[/]")
        if len(pairs) > 1:
            master_log.info("Resolution pass — %s @ %s", r, res)

        # STEP 1 — Warmup (states are per-resolution)
        ...
        # STEP 8 — FIM after forecast phase only (90m/30m sites; 900m skips)
        try:
            ...
            if LR_run:
                console.info("[bold]STEP 8:[/] FIM after forecast (90m/30m, QPE accums) …")
                run_fim_for_cycle(
                    regions_to_run=[r],
                    cycle=cycle_ts,
                    config=config,
        ...
        if bool(getattr(config, "postprocess_outputs", True)):
            try:
                from tito_utils.postprocess.output_processor import (
                    process_cycle_outputs,
                )

                rkey = region_path_key(r, res)
                cycle_ts = cycle_time.strftime("%Y%m%d.%H%M%S")
                if region_configs:
                    for _cfg in region_configs.values():
                        if _cfg.get("output_timestamp_str"):
                            cycle_ts = _cfg["output_timestamp_str"]
                            break
                console.info("[bold]Postprocess:[/] ensemble/deterministic summaries …")
                process_cycle_outputs(
                    cycle_ts,
                    rkey,
                ...
                if master_log:
                    master_log.error("postprocess failed: %s", _pp_exc)

    try:
        from tito_utils.postprocess.archive_manager import manage_archives

        console.info("[bold]Archive:[/] states/outputs retention …")
        manage_archives(
            config,
            cycle_time,
            states_path=statesPath,
        ...
        print(f"    archive skipped (non-fatal): {_ar_exc}")
        if master_log:
            master_log.error("archive failed: %s", _ar_exc)

    # Clear live precip once after ALL resolution passes (config-gated).
    if bool(getattr(config, "clear_precip_after_cycle", True)):
        console.info("[bold]Cleanup:[/] clearing live precip after cycle …")
        try:
            from tito_utils.file_utils.cleanup import clear_live_precip

            clear_live_precip(config, cycle_time, master_log=master_log)
        except Exception as _clr_exc:
            print(f"    Precip cleanup skipped (non-fatal): {_clr_exc}")
            if master_log:
                master_log.error("Precip cleanup failed: %s", _clr_exc)
