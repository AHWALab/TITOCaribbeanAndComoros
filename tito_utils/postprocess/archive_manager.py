...
_HOURLY_LOG = re.compile(r"^tito_hourly_(\d{8}T\d{6})\.log$")
_PIPELINE_LOG = re.compile(r"^pipeline_(\d{8})_(\d{4})_(\d{8})_(\d{6})\.log$")
...
def _parse_log_time(name: str):
    m = _HOURLY_LOG.match(name)
    if m:
        try:
            return datetime.strptime(m.group(1), "%Y%m%dT%H%M%S")
        except ValueError:
            return None
    m = _PIPELINE_LOG.match(name)
    if m:
        try:
            return datetime.strptime(m.group(3) + m.group(4), "%Y%m%d%H%M%S")
        except ValueError:
            return None
    return None


def _purge_logs(data_path: str, cutoff: datetime) -> int:
    log_dir = os.path.join(str(data_path).rstrip("/\\"), "logs")
    removed = 0
    if not os.path.isdir(log_dir):
        return 0
    for name in os.listdir(log_dir):
        if name in ("tito_cron.lock",) or not name.endswith(".log"):
            continue
        ts = _parse_log_time(name)
        if ts is None or ts >= cutoff:
            continue
...
def manage_archives(
    ...
    *,
    ...
    """Drop states older than ``states_keep_hours``, output cycles older
    than ``outputs_keep_hours``, and logs older than ``logs_keep_hours``."""
    ...
    log_h = float(getattr(config, "logs_keep_hours", 100) or 100)
